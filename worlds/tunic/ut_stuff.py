...
class UTMixin(World):
    ...
    def setup_options_from_slot_data(self, world: "TunicWorld") -> None:
        if hasattr(world.multiworld, "re_gen_passthrough"):
            if "TUNIC" in world.multiworld.re_gen_passthrough:
                self.using_ut = True
                self.passthrough = world.multiworld.re_gen_passthrough["TUNIC"]
                world.options.start_with_sword.value = self.passthrough["start_with_sword"]
                world.options.keys_behind_bosses.value = self.passthrough["keys_behind_bosses"]
                world.options.sword_progression.value = self.passthrough["sword_progression"]
                world.options.ability_shuffling.value = self.passthrough["ability_shuffling"]
                world.options.laurels_zips.value = self.passthrough["laurels_zips"]
                world.options.ice_grappling.value = self.passthrough["ice_grappling"]
                world.options.ladder_storage.value = self.passthrough["ladder_storage"]
                world.options.ladder_storage_without_items = self.passthrough["ladder_storage_without_items"]
                world.options.lanternless.value = self.passthrough["lanternless"]
                world.options.maskless.value = self.passthrough["maskless"]
                world.options.hexagon_quest.value = self.passthrough["hexagon_quest"]
                world.options.hexagon_quest_ability_type.value = self.passthrough.get("hexagon_quest_ability_type", 0)
                world.options.entrance_rando.value = self.passthrough["entrance_rando"]
                world.options.shuffle_ladders.value = self.passthrough["shuffle_ladders"]
                # world.options.shuffle_fuses.value = self.passthrough.get("shuffle_fuses", 0)
                # world.options.shuffle_bells.value = self.passthrough.get("shuffle_bells", 0)
                world.options.grass_randomizer.value = self.passthrough.get("grass_randomizer", 0)
                world.options.breakable_shuffle.value = self.passthrough.get("breakable_shuffle", 0)
                ...
                world.options.decoupled = self.passthrough.get("decoupled", 0)
                world.options.laurels_location.value = LaurelsLocation.option_anywhere
                world.options.combat_logic.value = self.passthrough.get("combat_logic", 0)
            else:
    ...
    def setup_found_entrances_datastorage(self) -> None:
        self.found_entrances_datastorage_key = []
        for portal_sdt1, portal_sdt2 in self.passthrough["Entrance Rando"].items():
            self.found_entrances_datastorage_key.append("Slot:{player}:" + self.modify_shop_key(portal_sdt1))
            # type issue here because it actually wants TunicWorld
            if not self.options.decoupled:
                self.found_entrances_datastorage_key.append("Slot:{player}:" + self.modify_shop_key(portal_sdt2))

    def disconnect_entrances(self) -> None:
        self.disconnected_entrances = {}
        for entrance in self.get_entrances():
            if entrance.name.startswith("Shop Portal") and entrance.connected_region is not None:
                self.disconnected_entrances[entrance] = entrance.connected_region
                entrance.connected_region = None
                continue
            # LS entrances start with the same name as the non-LS portal entrance
            name = entrance.name.split(" (LS) ", 1)[0]
            for portal in portal_mapping:
                if portal.name == name and entrance.connected_region is not None:
                    self.disconnected_entrances[entrance] = entrance.connected_region
                    entrance.connected_region = None
    ...
    def reconnect_found_entrance(self, portal_sdt: str) -> None:
        portal_name = self.get_entrance_name_from_sdt(portal_sdt)
        entrance_connected = False
        for entrance, region in self.disconnected_entrances.items():
    ...
    def reconnect_found_entrances(self, key: str, value: Any) -> None:
        if not value:
            ...
            self.reconnect_found_entrance(key.split(":")[-1])

    def connect_entrances(self) -> None:
        if self.using_ut and self.multiworld.enforce_deferred_connections in ("on", "default"):
            self.disconnect_entrances()
            self.setup_found_entrances_datastorage()
