...
class OnePerWorld(Toggle):
    """If enabled, makes it so one world unlock will go to each world. Not very tested, will probably break with some options combinations.
    It will almost certainly not work if you have multiple copies of a world in the pool."""


class OnePerWorldDistance(Range):
    """Decide how many spheres or fewer it can be between a world getting unlocked and the next world getting unlocked.
    It'll just crash gen if it fails."""
    range_start = 1
    range_end = 100
    default = 5


class OnePerWorldEnableDistance(Toggle):
    """Makes it so one_per_world will crash if a game and its unlock item aren't within a certain sphere distance
    of each other"""
...
class OnePerWorldCompletionDistance(Range):
    """Set the number of spheres between a world unlocking and when the world is beatable that you'll allow.
    If the number of spheres is less than specified, it will crash generation."""
    range_start = 0
    range_end = 5
    default = 1
...
class SlotLockWorld(AutoWorld.World):
    ...
    def create_items(self) -> None:
            ...
            if world.player_name in slots_to_lock:
                ...
                for i in range(min(10, self.options.unlock_item_copies.value + self.options.unlock_item_filler.value)):
                    ...
                    if i < self.options.unlock_item_copies.value and world.player_name not in self.associated_worlds:
                        itempool.append(self.create_slotlock_item(world.player_name))
                    else:
                ...
                if world.player_name in self.options.associated_worlds.keys():
                    for associated_world in self.options.associated_worlds[world.player_name]:
                        if associated_world in self.associated_worlds:
                            ...
                            index = -1
                            while itempool[index].name != "Nothing":
                                index -= 1
                            itempool.pop(index)
        ...
        for bonusSlot in range(self.options.bonus_item_slots.value):
            ...
            for bonusDupes in range(min(self.options.bonus_item_copies.value + self.options.bonus_item_filler.value, 10)):
                if bonusDupes < self.options.bonus_item_copies.value:
                    itempool.append(self.create_bonus_key(bonusSlot))
                else:
                    itempool.append(self.create_item("Nothing"))
        ...
        if self.options.one_per_world:
            self.world_unlock_items = [item for item in itempool if item.name.startswith("Unlock")]
            remaining_pool = [item for item in itempool if not item.name.startswith("Unlock")]
    ...
    def stage_pre_fill(cls, multiworld: MultiWorld):
        for self in multiworld.get_game_worlds(cls.game): #workaround this being a classmethod lol
            ...
            if self.options.one_per_world:
                self.worlds_to_unlocks = {}
                sphere_one_locs = self.multiworld.get_reachable_locations(CollectionState(self.multiworld))
                self.random.shuffle(self.world_unlock_items)
                first_item: LockItem = self.world_unlock_items.pop()

                first_loc: Location = self.random.choice(sphere_one_locs)
                while first_loc.item:
                    first_loc = self.random.choice(sphere_one_locs)

                first_loc.place_locked_item(first_item)

                def find_world_for_item(item: LockItem) -> AutoWorld.World:
                    slot_name = item.name.split("Unlock ")[-1]
                    for world in self.multiworld.worlds.values():
                        if world.player_name == slot_name:
                            return world

                slot1 = first_loc.player

                prev_world = find_world_for_item(first_item)
                slot2 = prev_world.player
                self.worlds_to_unlocks[slot1] = slot2

                while len(self.world_unlock_items) > 0:
                    locs = list(prev_world.get_locations())
                    loc = self.random.choice(locs)
                    while loc.item:
                        loc = self.random.choice(locs)
                    next_item: LockItem = self.world_unlock_items.pop()
                    # print(f"loc is {loc}")
                    # print(f"item is {next_item}")
                    loc.place_locked_item(next_item)
                    prev_world = find_world_for_item(next_item)
        ...
        for item in self.multiworld.get_items():
            if item.player == self.player:
                item_locations[item.name] = list(map(lambda loc: (loc.player, loc.address),
                                                     self.multiworld.find_item_locations(item.name, self.player, True)))

        if self.options.one_per_world:
            # do the sphere checking here
            def find_unlock_item_for_game(slot: int) -> Item:
                world = self.multiworld.worlds[slot]
                item_name = f"Unlock {world.player_name}"
                # print(item_name)
            ...
            for game1, game2 in self.worlds_to_unlocks.items():
                try:
                    game1_unlock_sphere, _, _ = find_sphere_for_item(find_unlock_item_for_game(game1))
                except StopIteration:
                    game1_unlock_sphere = 0  # probably when the item isn't in the pool, so it's unlocked from the start
                game2_unlock_sphere, game2_sphere_locs, game2_remaining_spheres = find_sphere_for_item(find_unlock_item_for_game(game2))
                # print(game1_unlock_sphere)
                # print(game2_unlock_sphere)
                # print(game2_unlock_sphere - game1_unlock_sphere)
                ...
                if (self.options.one_per_world_enable_distance
                    ...
                    raise OptionError(f"SlotLock: one_per_world_distance is lower than the distance between where a "
                                      f"game unlocked and the unlock in that game.\n"
                                      f"{game1_slot_name} unlocks at sphere {game1_unlock_sphere}.\n"
                                      f"{game2_slot_name} unlocks at sphere {game2_unlock_sphere}.\n")
